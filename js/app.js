...
const REPO = "darkxlmao/Chatbot";
const TOKEN = "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";

async function getFile() {
  const url = `https://api.github.com/repos/${REPO}/contents/data.txt`;
...
async function searchFile(query) {
  ...
  const match = lines.find(l => l.startsWith(query + "||"));
...
bot.on("document", async (msg) => {
  ...
  const entry = `${name}||${fileName}||${fileId}||${msg.from.username}||${Date.now()}`;

  await saveEntry(entry);

  bot.sendMessage(msg.chat.id, "File saved in database ✅");
});
bot.onText(/\/get (.+)/, async (msg, match) => {
  ...
  bot.sendDocument(msg.chat.id, result.file_id);
});
// IndexedDB Wrapper
